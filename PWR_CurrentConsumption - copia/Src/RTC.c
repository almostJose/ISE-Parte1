...
//  RTC_TimeTypeDef stimestructureget;
  ...
  periodic_id = osTimerNew(periodic_Callback, osTimerPeriodic, NULL, NULL);
	...
	tid_sleep = osThreadNew(ThSleep, NULL, NULL);
	LEDTimer_id = osTimerNew(LEDTimer_Callback, osTimerPeriodic, NULL, NULL);
}
...
static void LEDTimer_Callback (void *argument) {
	...
	if(cnt == 15)
	{
		osThreadFlagsSet(tid_sleep, 0x01);
	}
}
